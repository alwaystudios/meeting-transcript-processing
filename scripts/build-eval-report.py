...
def build_cases(job_results, datasets, fixture_meta):
    cases = {}
    for run_index, result in enumerate(job_results):
        version_key = f"run{run_index + 1}"
...
<script id="run-labels" type="application/json">__RUN_LABELS__</script>
<script>
(function () {
  const cases = JSON.parse(document.getElementById('eval-data').textContent);
  const runLabels = JSON.parse(document.getElementById('run-labels').textContent);
  const numRuns = runLabels.length;
  const versionKeys = Array.from({ length: numRuns }, (_, i) => 'run' + (i + 1));
  ...
  document.getElementById('meta-row').innerHTML =
    ...
    `<span>Runs compared: <b>${numRuns}</b></span>`;

  function stepsFor(list) {
    return versionKeys.map(vk => {
  ...
  function progCard(title, list) {
    ...
    const html = steps.map((s, i) => {
      const inner = !s ? '' : `<div class="prog-score ${s.pass === s.total ? 'pass-full' : 'pass-partial'}">${s.pass}/${s.total}</div><div class="prog-label">${esc(runLabels[i])}</div>`;
  ...
  function versionBlock(vk, i, v) {
    if (!v) return '';
    return `
      <div class="version-block">
        <div class="version-head">
          <span class="vname">${esc(runLabels[i])}</span>
  ...
  function caseHtml(c) {
    ...
    const versionsHtml = versionKeys.map((vk, i) => versionBlock(vk, i, c.versions[vk])).join('');
    return `
      <details class="case">
        <summary>
...
def main():
    ...
    print(f"wrote {out_path} ({len(ordered)} fixtures, {len(job_results)} run(s) compared)")
